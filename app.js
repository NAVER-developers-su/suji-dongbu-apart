...
const HELP = {
  ...
  bangsik: {
    ...
    body:
      ...
      '<p><b>③ 간이계산 (기본값)</b> — 2026.08.01 조합이 새로 내놓은 답변 시트의 산식입니다. 미납이자 풀에 이자의 이자 금리 3.825%가 매월 붙는 구조로, 1억·55개월 기준 이자 총 26,405,017원(단리분 24,200,000원 + 이자의 이자 2,205,017원)입니다.</p>' +
  ...
  ioni: {
    ...
    body:
      ...
      '<p>구조는 2026.08.01 조합 간이계산 시트로 확정됩니다 — <b>미납이자 풀에 이자의 이자 금리(3.825%)가 매월 붙는 구조</b>입니다.</p>' +
      '<code>이자총액 = 원금 × (대출금리÷이자의이자금리) × [(1+이자의이자금리÷12)ⁿ − 1]</code>' +
      '<p>이 이자총액에서 단리분(원금 × 대출금리 ÷ 12 × n)을 뺀 나머지가 이자의 이자입니다. 이주비·추가사업비 모두 이 구조(간이계산)를 기본값으로 쓰며, <b>단리</b>를 고르면 이자의 이자는 0입니다.</p>' +
      '<div class="helpsheet__note">참고: 간이계산 시트에 "(확인 필)" 표기가 있어 완전히 확정된 값은 아닙니다. 은행 확정 계산서는 아직 나오지 않았습니다.</div>',
  },
  mingam: {
    title: '금리 민감도',
    body:
  ...
  ioirate: {
    title: '이자의 이자 금리',
    body:
      '<p>만기까지 안 낸 이자가 매달 쌓이는데, 그 <b>쌓인 이자 풀에 매달 붙는 금리</b>입니다. 조합이 사업비 대출을 받아 이자를 대납하기 때문에 사업비 대출 이율이 적용됩니다.</p>' +
      '<code>이자의 이자(월) = 전월까지 쌓인 이자 × 이 금리 ÷ 12</code>' +
      '<div class="helpsheet__note">참고: 2026.08.01 조합 간이계산 시트의 고정 가정 3.825%입니다. "(확인 필)" 표기가 있어 확정값은 아닙니다.</div>',
  },
};
...
const STEPPER_CONFIG = {
  ...
  setIoiRate: { step: 0.1, min: 0, max: 20, decimals: 3, key: 'ioiRate' },
};
...
// 조합 간이계산(2026.08.01): 미납이자 풀에 이자의 이자 금리가 매월 붙는 구조의 닫힌 형태.
function proxyFactor(rLoan, rIoi, n) {
  if (rIoi <= 0) return rLoan / 12 * n; // 이자의 이자 금리가 0이면 단리와 같다
  const i = rIoi / 12;
  return (rLoan / rIoi) * (Math.pow(1 + i, n) - 1);
}
...
function computeAll(unit, s) {
  ...
  // 추가사업비 이자계수: 간이계산(2026.08.01 조합 답변 시트 — 미납이자에 이자의
  // 이자 금리가 매월 붙는 구조)을 기본으로 쓰거나, 조합 발표치(2026.07.30 안내문의
  // 1억당 정률)나 조합 공식 계산시트 그대로 단리(원금 × 연이율 ÷ 12 × 개월)를 쓴다.
  const addFactor =
    (s.addBasis === 'proxy') ? proxyFactor(rAdd, rIoi, nAdd) :
  ...
  // proxy: 조합 간이계산(2026.08.01) — 미납이자 풀에 이자의 이자 금리(rIoi)가
  //        매월 붙는 구조. proxyFactor의 닫힌 형태에서 단리분을 뺀 나머지가
  //        이자의 이자다.
  const moveIoi = (s.moveIoi === 'proxy')
    ? Math.round(moveAmount * (proxyFactor(rMove, rIoi, nMove) - moveFactor))
    : 0;
...
function updateResults(r) {
  ...
  document.getElementById('headAddRate').textContent =
    (state.addBasis === 'proxy') ? `연 ${r.addRatePct.toFixed(2)}% + 이자의 이자 ${state.ioiRate.toFixed(3)}%` :
...
function updateSettingsPanel(r) {
  ...
  document.getElementById('rateBreakdown').innerHTML =
    ...
    `<br>이자의 이자 = 매달 쌓인 이자 풀 × 이자의 이자 금리 ÷ 12 (조합 간이계산 구조)`;
}
...
function bindEvents() {
  ...
  document.getElementById('resetBtn').addEventListener('click', () => {
    ...
    state.ioiRate = DEFAULTS.ioiRate;
    ...
    document.getElementById('setIoiRate').textContent = DEFAULTS.ioiRate.toFixed(3);
